...
void UCommonUIActionRouter::ApplyUIInputConfig(const FUIInputConfig& NewConfig, bool bForceRefresh)
{
	...
	if (!GameViewportClient || !PC)
	{
		...
		UE_LOG(LogTemp, Error, TEXT("Missing GameViewportClient (%s) or PC (%s)"),
			( GameViewportClient ? TEXT("true") : TEXT("false"), ( PC ? TEXT("true") : TEXT("false")) ));
		return;
	}
